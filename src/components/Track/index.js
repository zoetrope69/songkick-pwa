...
export default class Track extends Component {
  ...
  sanitizeString(string) {
    return this.removeDiacritics(string)
      ...
      .replace(/(dj)/g, '') // remove dj set info from name
    ...
    return new Promise((resolve, reject) => {
      return fetch(url)
        ...
        .then(data => {
          ...
          // console.info(`Results from Spotify with the query: "${query}".`, data.artists);

          const items = data.artists.items;

          let item = items.find(item => this.sanitizeString(item.name.toLowerCase()) === query.toLowerCase());
